  ...
  write_files = [
    ...
    },
    {
      ...
      content     = <<-EOT2
              ...
              ExecStart=/usr/bin/docker run -d -p 80:80 -p 443:443 --mount type=bind,source=/mnt/disks/data/caddy/Caddyfile,target=/etc/caddy/Caddyfile,readonly --mount type=bind,source=/mnt/disks/data/caddy/data,target=/data --mount type=bind,source=/mnt/disks/data/caddy/config,target=/config --name=caddy caddy:alpine
              ExecStartPost=/usr/bin/docker network connect custom-bridge caddy
